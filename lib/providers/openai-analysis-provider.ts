...
export const SYSTEM_INSTRUCTIONS = `You are ASSERT's semantic analysis provider. Convert the supplied evidence documents and draft recommendation into a reviewable, source-bound decision graph.
...
- Every source binding must declare bindingId, documentId, ordered segmentIds, semanticRole, and nullable emphasisSegmentIds. Never invent segment IDs. Never return source content, quotations, offsets, page labels, or sections; ASSERT materializes all of them locally.
...
- Bind every material numeric fact or policy to a numeric-evidence segment and set numericRole to its semantic role. Numeric values and unit fields are proposals only: ASSERT deterministically recovers their authoritative value and canonical unit from locally materialized exact evidence. Use null numericRole for nonnumeric and derived nodes.
...
- Propose the complete machine-readable unit on every numeric operand. UnitSpec has one field only: unit. A monthly device rate is unit currency-per-device-per-month; never reduce it to currency-per-device or currency. A duration is unit month or year, never scalar. ASSERT discards conflicting source-fact proposals and derives calculated-node units locally.
...
- A recommendation must have value null and use select-candidate. Every candidate declares its canonical label, numeric valueNodeId, and policy maximumValueNodeId; selectionDirection is minimum; tieResult is a candidate label or null for unresolved. You describe this operation but never choose the authoritative winner.
...
- Compact coherent example: calculation={"operation":"select-candidate","candidates":[{"label":"Option A","valueNodeId":"a-total","maximumValueNodeId":"budget"},{"label":"Option B","valueNodeId":"b-total","maximumValueNodeId":"budget"}],"selectionDirection":"minimum","tieResult":null,"outputUnit":"recommendation","displayFormula":"Select the minimum eligible candidate"}; graph.recommendation={"nodeId":"recommendation","original":"Option A","corrected":"Option B"}.
...
export class OpenAIAnalysisProvider implements AnalysisProvider {
  ...
  constructor(options: OpenAIProviderOptions = {}) {
    if (!options.client && !process.env.OPENAI_API_KEY) throw new AnalysisProviderError("configuration", "Live analysis is not configured.");
