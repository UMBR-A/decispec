...
import { DecisionGraphSchema, SourceSpanSchema } from "../lib/domain/schemas";
...
describe("ASSERT deterministic proof engine", () => {
  it("runtime-validates the complete proof-engine regression project", () => {
    expect(DecisionGraphSchema.parse(proofEngineProject.graph).id).toBe("school-device-proof");
    expect(SourceSpanSchema.array().parse(proofEngineProject.sourceSpans)).toHaveLength(11);
  });

  it("computes the imported memo expression exactly", () => {
    const result = evaluateGraph(proofEngineProject.graph, proofEngineSourceSpanIds);
  ...
  it("accepts monthly duration and rejects dimensionless duration for a monthly rate", () => {
    const monthly = applyCorrection(proofEngineProject.graph, proofEngineProject.graph.corrections[0]);
    const monthlyResult = evaluateGraph(monthly, proofEngineSourceSpanIds);
    expect(value(monthlyResult, "a-support")).toBe(208008);
    expect(status(monthlyResult, "a-support")).toBe("calculated");

    const dimensionless = resetGraph(proofEngineProject.graph);
    ...
    const dimensionlessResult = evaluateGraph(dimensionless, proofEngineSourceSpanIds);
  ...
  it("requires explicit deterministic conversion from years to months", () => {
    ...
    const result = evaluateGraph(graph, proofEngineSourceSpanIds);
  ...
  it("does not silently equate three years with three months", () => {
    const yearly = evaluateGraph(proofEngineProject.graph, proofEngineSourceSpanIds);
    const replacementCalculation = proofEngineProject.graph.corrections[0].replacementCalculation;
    ...
    const monthlyGraph = applyCorrection(proofEngineProject.graph, {
      ...proofEngineProject.graph.corrections[0],
    ...
    const monthly = evaluateGraph(monthlyGraph, proofEngineSourceSpanIds);
  ...
  it("applies a correction without mutating the original", () => {
    const originalJson = JSON.stringify(proofEngineProject.graph);
    const correctedGraph = applyCorrection(proofEngineProject.graph, proofEngineProject.graph.corrections[0]);
    const corrected = evaluateGraph(correctedGraph, proofEngineSourceSpanIds);
    ...
    expect(JSON.stringify(proofEngineProject.graph)).toBe(originalJson);
  });

  it("produces a semantic before/after diff", () => {
    const before = evaluateGraph(proofEngineProject.graph, proofEngineSourceSpanIds);
    const after = evaluateGraph(applyCorrection(proofEngineProject.graph, proofEngineProject.graph.corrections[0]), proofEngineSourceSpanIds);
  ...
  it("resets to an immutable copy", () => {
    const reset = resetGraph(proofEngineProject.graph);
    reset.nodes[0].statement = "changed";
    expect(proofEngineProject.graph.nodes[0].statement).toBe("Enrollment is 300 students.");
  });
  ...
  it("rejects missing operand nodes and materializes missing calculation edges", () => {
    const missingNode = resetGraph(proofEngineProject.graph);
    ...
    const missingEdge = resetGraph(proofEngineProject.graph);
  ...
  it("exposes missing-source errors", () => {
    const ids = new Set(proofEngineSourceSpanIds);
    ids.delete("s-a-support");
    const result = evaluateGraph(proofEngineProject.graph, ids);
  ...
  it("calculates the transparent weighted integrity score", () => {
    const broken = evaluateGraph(proofEngineProject.graph, proofEngineSourceSpanIds);
    const corrected = evaluateGraph(applyCorrection(proofEngineProject.graph, proofEngineProject.graph.corrections[0]), proofEngineSourceSpanIds);
  ...
  it("atomically replaces correction calculation edges while preserving semantic and policy edges", () => {
    ...
    const replacement = structuredClone(proofEngineProject.graph.corrections[0].replacementCalculation);
    ...
    const corrected = applyCorrection(original, { ...proofEngineProject.graph.corrections[0], replacementCalculation: replacement });
    ...
    expect(applyCorrection(original, { ...proofEngineProject.graph.corrections[0], replacementCalculation: replacement })).toEqual(corrected);
  });

  it("rejects missing, self-referential, and cycle-creating correction operands", () => {
    const correction = structuredClone(proofEngineProject.graph.corrections[0]);
    ...
    expect(() => applyCorrection(proofEngineProject.graph, correction)).toThrow(expect.objectContaining({ code: "CORRECTION_OPERAND_NODE_MISSING" }));
    correction.replacementCalculation.operands[0] = { kind: "ref", nodeId: "a-support" };
    expect(() => applyCorrection(proofEngineProject.graph, correction)).toThrow(expect.objectContaining({ code: "CORRECTION_SELF_DEPENDENCY" }));
    correction.replacementCalculation.operands[0] = { kind: "ref", nodeId: "a-total" };
    expect(() => applyCorrection(proofEngineProject.graph, correction)).toThrow(expect.objectContaining({ code: "CORRECTION_CREATES_CYCLE" }));
  });
  ...
  it("exports the effective evaluated dependency structure", () => {
    ...
    const evaluated = evaluateGraph(graph, proofEngineSourceSpanIds);
    const proof = buildProofExport(proofEngineProject.id, graph, evaluated, evaluated, proofEngineProject.sourceSpans, { phase: "verified", generatedAt: "2026-07-18T00:00:00.000Z" });
