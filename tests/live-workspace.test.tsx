...
describe("live analysis workspace", () => {
  it("calls live analysis only after explicit action and renders engine-derived results", async () => {
    ...
    await screen.findByText("Live analysis ready");
  ...
  it("previews, applies, undoes, and reports deterministic live corrections", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Paste evidence text/i), { target: { value: "Evidence passage." } });
    fireEvent.click(screen.getByTestId("test-live-decision"));
    expect(await screen.findByTestId("live-correction-preview")).toBeInTheDocument();
    fireEvent.click(screen.getByTestId("apply-live-correction"));
    expect(await screen.findByRole("heading", { name: "Decision verified" })).toBeInTheDocument();
    fireEvent.click(screen.getByTestId("view-live-report"));
    expect(await screen.findByTestId("live-proof-report")).toHaveTextContent("PROOF PASSED");
    expect(screen.getByTestId("live-proof-report")).toHaveTextContent("Vendor B");
    fireEvent.click(screen.getByRole("button", { name: /Undo correction/i }));
    expect(await screen.findByRole("heading", { name: "Decision broken" })).toBeInTheDocument();
  });
});
